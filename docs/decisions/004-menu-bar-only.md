...
- Keep only the local auth reader, read-only ChatGPT usage and reset-credit detail requests on the same host, quota classification, and a small status menu with native remaining-quota and reset-time progress bars plus the available reset-credit count and next expiry.
