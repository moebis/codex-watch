...
Codex Spark rows default hidden. The user-requested `Show Codex Spark Stats` preference persists only visibility and rebuilds the menu without fetching. Recognize adjacent Codex/Spark words in IDs or titles, including versioned GPT names and duplicate suffixes; preserve unrelated model names and base-weekly quota semantics.
