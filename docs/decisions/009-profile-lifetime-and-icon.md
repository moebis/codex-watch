...
- Hide Codex Spark rows by default while preserving them in the decoded capability model. The user-requested `Show Codex Spark Stats` menu preference persists an explicit on/off choice and rebuilds the menu without a refresh. Recognize adjacent Codex/Spark words in identifiers or titles so legacy names, versioned GPT names, and duplicate suffixes behave consistently.
